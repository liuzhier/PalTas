...
public static partial class TasScript
{
    ...
    public static async Task InitGlobalSceneEventAsync(CancellationToken token)
    {
        ...
        // 见石碑篇_初登岛_过迷阵
        {
            for (var i = _18_阿修罗石像A; i <= _18_阿修罗石像F; i++)
            {
                var 阿修罗石像 = GetEvent(18, i);
                阿修罗石像.TriggerMode = Core.EventTriggerMode.TouchFar;
                阿修罗石像.TriggerScript = 0x13A0;
                SetEventInfo(18, i, 阿修罗石像);
            }
            ...
            雪莲子.X = 1392;
            雪莲子.Y = 424;
            雪莲子.TriggerMode = Core.EventTriggerMode.TouchNear;
            雪莲子.TriggerScript = 五张灵符.TriggerScript;
            五张灵符.TriggerScript = 0x0000;
