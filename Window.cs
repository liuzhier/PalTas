...
public static unsafe class TasWindow
{
    /// <summary>
    /// 窗口句柄
    /// </summary>
    ...
    {
#if DEBUG || true
        // 先关闭MSI Afterburner相关进程
        Log("关闭MSI Afterburner相关进程...");
        KillProcessByName("Pal");
        KillProcessByName("MSIAfterburner");
        KillProcessByName("RTSS");
        KillProcessByName("RTSSHooksLoader64");
#endif // DEBUG
...
#if DEBUG || true
        // 重新启动 MSI Afterburner
        Log("重新启动 MSI Afterburner...");
        Process.Start(new ProcessStartInfo
        {
            FileName = "MSIAfterburner.exe",
            WorkingDirectory = @"E:\Program Files (x86)\MSI Afterburner\",
            UseShellExecute = true
        });
#endif // DEBUG
    }
    ...
    public static bool PressKey(User32.VK key)
    {
        // 如果窗口为前台活动窗口，才发送按键事件
    ...
    public static bool ReleaseKey(User32.VK key)
    {
        // 如果窗口为前台活动窗口，才发送按键事件
        if ((User32.GetForegroundWindow() != WindowHandle)) return false;
